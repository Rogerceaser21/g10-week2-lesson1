...
#
# By default this script publishes g10-week2-lesson1.html exactly as it
...
if [ "$FROM_DOWNLOADS" -eq 1 ]; then
  NEWEST=$(ls -t "$HOME"/Downloads/g10-week2-lesson1*.html 2>/dev/null | head -1 || true)
  if [ -z "$NEWEST" ]; then
    echo "--from-downloads was asked for, but there is no g10-week2-lesson1*.html in ~/Downloads."
    exit 1
  fi
